...
import { browserSupabase, handleInvoiceTypeError, enhanceErrorMessage } from '@/lib/supabase';
              ...
              <Card className="p-6">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50 dark:bg-gray-700">
                        <tr>
                          ...
                          <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Price (RM)</th>
                          <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total (RM)</th>
                        ...
                        {selectedInvoice.items.map((item) => (
                          <tr key={item.id}>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <Input value={item.description} onChange={(e) => updateInvoiceItem(item.id, 'description', e.target.value)} className="w-full" />
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <Input type="number" value={item.quantity} onChange={(e) => updateInvoiceItem(item.id, 'quantity', parseFloat(e.target.value) || 0)} className="w-full" />
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <Input type="number" value={item.price} onChange={(e) => updateInvoiceItem(item.id, 'price', parseFloat(e.target.value) || 0)} className="w-full" />
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">RM {(item.quantity * item.price).toFixed(2)}</td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <Button variant="ghost" size="sm" onClick={() => removeInvoiceItem(item.id)}>Remove</Button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                    ...
                    <Button variant="outline" onClick={addInvoiceItem} className="mt-4">Add Item</Button>
                  </div>
                ...
                <div className="overflow-x-auto">
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200">
                      ...
                      {invoices.filter(invoice => activeTab === 'all' || invoice.status === activeTab).map((invoice) => (


                        <tr key={invoice.id}>


                          <td className="px-6 py-4 whitespace-nowrap">{invoice.type === 'invoice' ? 'Invoice' : 'Quotation'}</td>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">{formatDateDisplay(invoice.date)}</td>


                          <td className="px-6 py-4 whitespace-nowrap">{invoice.type === 'invoice' ? formatDateDisplay(invoice.due_date) : '-'}</td>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                              ...
                              'bg-yellow-100 text-yellow-800'}`}>{invoice.status.charAt(0).toUpperCase() + invoice.status.slice(1)}</span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">RM {parseFloat(String(invoice.total)).toFixed(2)}</td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex space-x-2">
                              <Button variant="outline" size="sm" onClick={() => { handleEditInvoice(invoice); setIsInvoiceType(invoice.type === 'invoice'); }} disabled={loading}>Edit</Button>
                              <Button variant="outline" size="sm" onClick={() => { setSelectedInvoice(invoice); setShowPDF(true); setIsInvoiceType(invoice.type === 'invoice'); }} disabled={loading}>View PDF</Button>
                              <Button variant="outline" size="sm" onClick={() => handleDeleteInvoice(invoice.id)} disabled={loading}>Delete</Button>
                            </div>
                          </td>
                        </tr>
